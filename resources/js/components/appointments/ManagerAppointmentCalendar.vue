...
<template>
    <div class="cal-wrapper">
        <div class="cal-header">
            <button class="cal-nav" @click="store.prevMonth()" aria-label="Previous month">
                <i class="fa-solid fa-chevron-left" />
            </button>
            <h2 class="cal-month">{{ monthName }}</h2>
            <button class="cal-nav" @click="store.nextMonth()" aria-label="Next month">
        ...
        <div v-else class="cal-grid">
            ...
            >
                ...
                <div v-else class="cal-badges">
                    <span
                        v-for="(count, status) in getDayBadges(day)"
                        :key="status"
                        class="cal-badge"
...
const monthName = computed(() => {
    const date = new Date(store.currentYear, store.currentMonth - 1, 1)
    return new Intl.DateTimeFormat(locale.value, { month: 'long' }).format(date)
})

const weekDays = computed(() => {
    const formatter = new Intl.DateTimeFormat(locale.value, { weekday: 'short' })
    // Usamos una semana base para extraer las abreviaciones relativas al locale activo
    return [0, 1, 2, 3, 4, 5, 6].map(dayOffset => {
        const d = new Date(2026, 7, 2 + dayOffset) // Domingo 2 de Agosto de 2026
...
function isDayOff(day) {
    return !!store.daysOff[key(day)]
}

function getDayOff(day) {
    return store.daysOff[key(day)] ?? null
}
