...
const paginationRange = computed(() => {
    ...
    // El número del medio es la página activa, pero nunca puede
    // pisar a los 2 primeros ni a los 2 últimos
    const middle = Math.min(Math.max(current, 3), total - 2)

    return [1, 2, middle, total - 1, total]
})
...
/* ── Paginación ── */
.pagination-navbar { display: flex; justify-content: center; padding: 5px 20px 15px; border-top: 1px solid #1e2a3a; }
.pagination { display: flex; align-items: center; gap: 6px; padding: 8px 16px; flex-wrap: nowrap; overflow-x: auto; max-width: 100%; }
.pagination a {
