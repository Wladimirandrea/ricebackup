...
<template>
    <div class="day-view">
        ...
        <div class="day-view__body">
            ...
            <div v-else-if="store.isDayOff" class="day-day-off-card">
                ...
                <div class="day-day-off-card__info">
                    <h3 class="day-day-off-card__title">
                        <template
                            v-if="store.dayOffInfo?.reason && $te(`appointments.reasons.${store.dayOffInfo.reason.toLowerCase().replace(/ /g, '_')}`)">
                            {{ $t(`appointments.reasons.${store.dayOffInfo.reason.toLowerCase().replace(/ /g, '_')}`) }}
                        </template>
                        <template v-else-if="store.dayOffInfo?.reason">
                            {{ store.dayOffInfo.reason }}
                        </template>
                        <template v-else>
                            {{ $t('appointments.dayOffTitle') }}
                        </template>
                    </h3>
                    <p v-if="store.dayOffInfo?.reason" class="day-day-off-card__reason">{{ store.dayOffInfo.reason }}
                    </p>
                    <p class="day-day-off-card__time">{{ store.dayOffInfo?.start_time }} — {{ store.dayOffInfo?.end_time
                        }}</p>
                </div>
            </div>
                    ...
                    <div class="day-timeline-header">
                        ...
                        <div class="day-timeline-header__clients">
                            <div v-for="client in uniqueClients" :key="client.id" class="day-col-header">
                                ...
                                <span class="day-col-count">{{ clientAppts(client.id).length }} {{
                            ...
                            <div v-for="client in uniqueClients" :key="client.id" class="day-timeline__client-col">
                                <div class="day-col-slots">
                                    <div v-for="slot in timeSlots" :key="slot" class="day-slot"
                                        ...
                                        <div v-for="appt in getSlotAppt(client.id, slot)" :key="appt.id"
                                            class="day-appt" :class="`day-appt--${appt.status}`"
                                            ...
                                            <span class="day-appt__time">{{ appt.start_time }} — {{ appt.end_time
                                                }}</span>
                                        </div>
                                    </div>
                                </div>
        ...
        <!-- Modal detalle -->
        <ManagerAppointmentDetailModal v-model="showDetail" :appointment="selectedAppt"
            @status-changed="showDetail = false" @appointment-updated="onApptUpdated" />
    </div>
</template>
...
.day-timeline-wrapper {
    ...
    overflow: hidden;
    border-radius: 18px;
    background: linear-gradient(135deg, rgba(180, 180, 160, 0.35) 0%, rgba(160, 180, 140, 0.25) 40%, rgba(200, 160, 140, 0.25) 70%, rgba(160, 140, 180, 0.25) 100%);
...
.day-day-off-card__icon {
    font-size: 3rem;
}

.day-day-off-card__info {
    display: flex;
    flex-direction: column;
    gap: 6px;
}

.day-day-off-card__title {
    font-size: 1.1rem;
    font-weight: 700;
    color: #fff;
    margin: 0;
...
.day-day-off-card__reason {
    font-size: 0.9rem;
    color: rgba(255, 255, 255, 0.7);
    margin: 0;
}

.day-day-off-card__time {
    font-size: 0.82rem;
